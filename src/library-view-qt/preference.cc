...
PreferenceWidget::PreferenceWidget(QWidget* parent)
	...
	file_entry->setText(static_cast<const char*>(
	filename_contract(uri_to_filename(get_pref_uri()))));
...
void PreferenceWidget::apply() {
	if(instance.isNull())
	return;
	auto path = instance->file_entry->text();
	if(!test_uri(path)) {
		audqt::simple_message("Error", "Invalid library path.");
		return;
	}
	auto new_root = path.toUtf8();
	if(uri_to_filename(get_pref_uri()) != new_root) {
		set_pref_uri(new_root);
