...
public:
    ...
    static constexpr PluginInfo info = {N_("Mac Now Playing")};

    constexpr MacNowPlayingPlugin() : GeneralPlugin(info, false) {}
...
{
    // No need to change the now playing status since it will be cought be the
    // hook.
...
static void update_metadata(void * data, void * user)
{
    ...
    if (aud_drct_get_ready())
    {
        ...
        if (file)
        {
            AudArtPtr artwork = aud_art_request(file, AUD_ART_FILE);
...
const char MacNowPlayingPlugin::about[] =
    N_("Now Playing Plugin for Audacious\n"
       ...
       "It allows Audacious to be controlled throught media keys,"
       "to show the current album artwork and to seek the playback.");

bool MacNowPlayingPlugin::init()
{
    (void)remote_command_setup_handler();
    (void)remote_command_start();
    (void)remote_command_disable_commands();
