...
playlist_manager_populate ( GtkListStore * store )
{
    ...
    while ( playlists != NULL )
    {
        ...
        if(playlist != active) //XXX the active playlist has been locked in aud_playlist_new_from_selected()
...
playlist_manager_cb_new ( gpointer listview )
{
    ...
    /* this ensures that aud_playlist_manager_update() will
...
playlist_manager_cb_del ( gpointer listview )
{
    ...
    if ( gtk_tree_selection_get_selected( listsel , &store , &iter ) == TRUE )
    {
        ...
        {
            /* let aud_playlist_manager_update() handle the deletion of the last playlist */
...
playlist_manager_ui_show ( void )
{
    ...
    aud_hook_associate("playlist update", (HookFunction) playlist_manager_update, NULL);
}
