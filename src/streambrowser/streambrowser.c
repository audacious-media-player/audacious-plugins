...
static GMutex *update_thread_mutex = NULL; /* used to synchronize the access to the update queue */
GMutex *update_streamdir_mutex = NULL; /* used to synchronize the access to streamdirs */
...
static void gui_init ()
{
    ...
    update_streamdir_mutex = g_mutex_new ();
...
static void gui_done ()
{
    ...
    g_mutex_free (update_streamdir_mutex);
    update_streamdir_mutex = NULL;
                              ...
                              gboolean add_to_playlist)
{
    ...
    {
        if (!update_thread_data_queue) {
            AUDDBG("the plugin has been closed\n");
    ...
    while (data != NULL)
    {
        ...
        if (data->streaminfo != NULL)
        {
            ...
            else
            {
                ...
                {
                    g_mutex_lock (update_streamdir_mutex);
                    shoutcast_streaminfo_fetch (data->category,
                                                data->streaminfo);
                    g_mutex_unlock (update_streamdir_mutex);
                }
                /* xiph */
                else if (strncmp
