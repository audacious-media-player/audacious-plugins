...
{
    unsigned char c;
    if (fp.fread(&c, 1, sizeof c) != sizeof c)
    {
...
/* Read NTstring: reads null-terminated string from file.
 * Return 1 if error occurs.
 */
static int read_NTstring(VFSFile & fp, char s[])
{
    ...
    for (i = 0; i < AYEMU_VTX_NTSTRING_MAX &&
                (n = fp.fread(&c, 1, sizeof c)) == sizeof c && c;
         i++)
    {
        s[i] = c;
    }
...
bool ayemu_vtx_t::read_header(VFSFile & file)
{
    ...
    if (file.fread(buf, sizeof buf, 1) != 1)
    {
    ...
    {
        AUDERR("File %s is _not_ a VORTEX format!\n"
               "It does not begin with AY or YM.\n",
               file.filename());
        error = 1;
    }
    ...
    if (!error)
        error = read_byte(file, &hdr.stereo);
    if (!error)
        error = read_word16(file, &hdr.loop);
    if (!error)
        error = read_word32(file, &hdr.chipFreq);
    if (!error)
        error = read_byte(file, &hdr.playerFreq);
    if (!error)
        error = read_word16(file, &hdr.year);
    if (!error)
    {
    ...
    if (!error)
        error = read_NTstring(file, hdr.title);
    if (!error)
        error = read_NTstring(file, hdr.author);
    if (!error)
        error = read_NTstring(file, hdr.from);
    if (!error)
        error = read_NTstring(file, hdr.tracker);
    if (!error)
        error = read_NTstring(file, hdr.comment);
...
bool ayemu_vtx_t::get_next_frame(unsigned char * regs)
{
    ...
    for (int n = 0; n < 14; n++, p += numframes)
        regs[n] = *p;
...
{
    static const char * stereo_types[] = {
