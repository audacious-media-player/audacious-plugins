...
gint convert_process(gpointer ptr, gint length)
{
    ...
    convert_output = g_realloc (convert_output, FMT_SIZEOF (out_fmt) * length);

    if (in_fmt == out_fmt)
        memcpy (convert_output, ptr, FMT_SIZEOF (in_fmt) * length);
    else if (in_fmt == FMT_FLOAT)
        audio_to_int (ptr, convert_output, out_fmt, length);
    else if (out_fmt == FMT_FLOAT)
        audio_from_int (ptr, in_fmt, convert_output, length);
    else
    {
        temp = g_malloc (sizeof (gfloat) * length);
        audio_from_int (ptr, in_fmt, temp, length);
        audio_to_int (temp, convert_output, out_fmt, length);
        g_free (temp);
    }
