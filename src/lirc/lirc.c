...
void init_lirc(void)
{
	...
	input_tag = g_io_add_watch(g_io_channel_unix_new(lirc_fd), G_IO_IN, lirc_input_callback ,NULL);
	...
	while((ret=lirc_nextcode(&code))==0 && code!=NULL)
	{
		while((ret=lirc_code2char(config,code,&c))==0 && c!=NULL)
		{
			...
			else if(strncasecmp("FWD",c,3)==0)
			{
				...
				playlist_time = aud_playlist_entry_get_length(playlist,playlist_pos, FALSE); 
