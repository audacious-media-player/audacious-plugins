...
#ifdef _WIN32
  AUDDBG("lHotkeyFlow:Win call: get_is_mod.%s", gdk_keyval_name(event->keyval));
#endif
  int mod = 0;
  int is_mod = 0;
...
void Hotkey::set_keytext(GtkWidget *entry, int key, int mask, int type) {
  ...
  if (key == 0 && mask == 0) {
    ...
    static const unsigned int modifiers[] = {HK_CONTROL_MASK, HK_SHIFT_MASK, HK_MOD1_ALT_MASK, HK_MOD2_MASK, HK_MOD3_MASK, HK_MOD4_MASK, HK_MOD5_MASK};
