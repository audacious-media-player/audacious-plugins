...
static void do_command(char *cmd, const char *current_file, int pos)
{
    ...
    if (cmd && strlen(cmd) > 0)
    {
        ...
        const char * artist = tuple ? tuple_get_string (tuple, FIELD_ARTIST, NULL) : NULL;
        if (artist)
            formatter_associate(formatter, 'a', artist);
        else
            formatter_associate(formatter, 'a', "");

        const char * album = tuple ? tuple_get_string (tuple, FIELD_ALBUM, NULL) : NULL;
        if (album)
            formatter_associate(formatter, 'b', album);
        else
            formatter_associate(formatter, 'b', "");

        const char * title = tuple ? tuple_get_string (tuple, FIELD_TITLE, NULL) : NULL;
        if (title)
